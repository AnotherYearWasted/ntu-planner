...
public class ColoredGraph {
    ...
    private boolean backtrackCliques(int layer, ArrayList<Node> remaining) {
        ...
        // basically, iterate through all nodes with the current color, and check if they can be added to the clique.
        // just brute force

        // a feeble attempt at optimization :) trying to skip whatever nodes that have the same color as the layer
        int nextColor = 0;
        while (nextColor < remaining.size() && remaining.get(nextColor).color == layer)
            ++nextColor;
        ...
        for (int i = 0; i < nextColor; i++) {
            ...
            for (int j = nextColor; j < remaining.size(); j++) {
                if (remaining.get(j).isAdjacent(current)) {
                    if (remaining.get(j).color - last > 1)
                        break;
            ...
            if (last == colors - 1) {
                ...
                nextRemaining.sort(
                        Comparator.comparingInt((Node node) -> node.color).thenComparingInt(node -> -node.getDegree()));
                if (backtrackCliques(layer + 1, nextRemaining))
                    return true;
                for (int j = 0; j < remaining.size(); j++) {
                    if (!remaining.get(j).isAdjacent(current))
